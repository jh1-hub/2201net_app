...
import { LectureMode } from './components/LectureMode.js';
import { MISSION_SETS } from './data/missions.js';
import { LECTURE_SLIDES } from './data/lectures.js';
import { generateId, isConnected, findPath, isValidIP } from './utils.js';
import { CheckCircle, ArrowRight, AlertCircle, RefreshCw, LayoutTemplate, X, Home, Play, Network, BookOpen, HelpCircle, LogOut, Lightbulb, GraduationCap, School, Map } from 'lucide-react';
...
const App = () => {
  ...
  // Lecture Flow State
  const [isLectureFlow, setIsLectureFlow] = useState(true);
  const [lecturePhase, setLecturePhase] = useState(1);
  ...
  const [isCurrentMissionCompleted, setIsCurrentMissionCompleted] = useState(false); // New state to track if current mission is done
  ...
  const startLectureMode = () => {
      resetSimulationState();
      setIsLectureFlow(true);
      setLecturePhase(1);
      setView('lecture');
  };

  // レクチャー（フェーズ毎）完了時の処理
  const handleLecturePhaseComplete = () => {
      if (lecturePhase === 1) {
          // Phase 1完了 -> シミュレーター(Mission 1)へ
          resetSimulationState();
          setMode('quiz');
          // 基本コースのミッション1を使用
          const basicCourse = MISSION_SETS.find(m => m.id === 'basic_course') || MISSION_SETS[0];
          setCurrentMissionSet(basicCourse);
          setCurrentMissionIndex(0); // Mission 1
          setView('simulation');
          log("実践: 機器を配置してみましょう");
      } else {
          // 全行程完了 -> ホームへ
  ...
  const resetSimulationState = () => {
    ...
    setIsCurrentMissionCompleted(false);
  ...
  const checkMissionCompletion = () => {
      ...
      if (currentMission.check(state)) {
          ...
          setIsCurrentMissionCompleted(true);
          setMissionError(null);
          setShowHintModal(false);
      } else {
  ...
  const nextMission = () => {
      setShowSuccessModal(false);
      setIsCurrentMissionCompleted(false); // Reset completion status for next mission
      setMissionFlags({ pingSuccess: false, encryptedSuccess: false });
      
      // まず、現在のセットに次のミッションがあるか確認
      if (currentMissionSet && currentMissionIndex < currentMissionSet.missions.length - 1) {
          setCurrentMissionIndex(prev => prev + 1);
          return;
      }

      // セットの全ミッション完了、かつレクチャーフロー中なら解説フェーズ2へ
      if (isLectureFlow && lecturePhase === 1) {
          setLecturePhase(2);
          setView('lecture');
          return;
      }
      
      // それ以外（単独ミッションモードの終了など）
      goToHome();
  };
  ...
  if (view === 'lecture') {
      // Phase 1: Slides 1-3 (Indices 0, 1, 2)
      // Phase 2: Slides 4-7 (Indices 3, 4, 5, 6)
      const isPhase1 = lecturePhase === 1;
      
      // Pass full array in Phase 2 to allow back navigation
      const slides = isPhase1 
          ? LECTURE_SLIDES.slice(0, 3) 
          : LECTURE_SLIDES;
      
      const initialIndex = isPhase1 ? 0 : 3;

      return html`<${LectureMode} 
          key=${lecturePhase} 
          onExit=${goToHome} 
          onComplete=${handleLecturePhaseComplete} 
          slides=${slides}
          offset=${0}
          totalSlides=${LECTURE_SLIDES.length}
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
        ...
        <main className="flex-1 max-w-6xl mx-auto w-full p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                  ...
                  <div className="flex items-center justify-between relative z-10">
                    <div>
                      <h3 className="text-lg font-bold text-slate-800 group-hover:text-emerald-600 transition-colors">解説・実践コース</h3>
                      <p className="text-slate-500 mt-1">基本知識の解説と、シミュレーター実習を交互に行います。</p>
                    </div>
          ...
          <div className="mb-12">
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-amber-300 transition-all relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-10">
                      <${Map} size=${64} className="text-amber-600" />
                  </div>
                  <div className="flex items-center justify-between relative z-10">
                    <div>
                      <h3 className="text-lg font-bold text-slate-800 mb-1">ジオロケーションにアクセス</h3>
                      <p className="text-slate-500 text-sm mb-4">IPアドレスから位置情報を特定する仕組みを体験できます。</p>
                      <a 
                          href="https://www.geolocation.com/ja/index" 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-bold text-sm transition-colors"
                      >
                          <${Map} size=${16} />
                          サイトを開く
                      </a>
  ...
  // Determine label for the proceed button
  const getProceedButtonLabel = () => {
      if (currentMissionSet && currentMissionIndex < currentMissionSet.missions.length - 1) {
          return "次のミッションへ";
      }
      if (isLectureFlow && lecturePhase === 1) {
      ...
      <div className="h-16 bg-white border-b border-slate-200 flex items-center px-4 justify-between shadow-sm z-10 shrink-0">
          ...
          ${mode === 'quiz' && currentMission ? html`
              ...
              <div className="flex items-center gap-2 md:gap-3 ml-2">
                   ${missionError && !isCurrentMissionCompleted && html`
                   ...
                   ${isCurrentMissionCompleted ? html`
                       <button 
                           onClick=${nextMission}
                           className="px-3 md:px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-sm font-bold flex items-center gap-2 transition-all active:scale-95 whitespace-nowrap text-sm animate-in fade-in"
                       >
                           <${ArrowRight} size=${18} />
                           <span className="hidden sm:inline">${getProceedButtonLabel()}</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200 p-4">
              <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl transform scale-100 text-center relative overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                       <button 
                          onClick=${() => setShowSuccessModal(false)}
                          className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold flex items-center justify-center gap-2 transition-all"
                        >
                      ...
                      ` : html`
                           <button 
                            onClick=${goToHome}
                            className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all"
                          >
                              ホームに戻る
                          </button>
                      `)}
